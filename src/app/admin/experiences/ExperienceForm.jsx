...
export default function ExperienceForm({ initialData = null }) {
  ...
  const [formData, setFormData] = useState({
    ...
    date_id: initialData?.date_id || "",
    date_en: initialData?.date_en || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        const uploadedUrl = await uploadImage(imageFile, 'experiences');
        if (uploadedUrl) {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-navy/70 text-sm mt-1">
            {isEdit ? "Perbarui informasi pengalaman yang sudah ada." : "Lengkapi form di bawah ini untuk menambahkan riwayat baru."}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-navy/5 p-6 md:p-8 flex flex-col gap-8">
        ...
        {/* Basic Global Info */}
        <div>
          <h2 className="text-lg font-bold text-navy border-b border-navy/5 pb-2 mb-4">Informasi Umum</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Nama Organisasi / Perusahaan</label>
              <input
                type="text"
                name="organization"
                required
                ...
                placeholder="Contoh: Politeknik Caltex Riau"
              />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Peran / Jabatan</label>
              <input
                type="text"
                name="title_id"
                required
            ...
            <div className="flex flex-col gap-2">
              <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Tanggal / Durasi</label>
              <input
                type="text"
                name="date_id"
                required
                ...
                onChange={handleChange}
                className="bg-navy/5 border border-navy/10 rounded-xl px-4 py-3 text-navy placeholder:text-navy/40 focus:outline-none focus:border-navy/30 focus:ring-1 focus:ring-navy/30 transition-all"
                placeholder="Contoh: Agu 2026 - Sekarang"
              />
            </div>
            <div className="flex flex-col gap-2 md:col-span-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Role / Position</label>
              <input
                type="text"
                name="title_en"
                required
            ...
            <div className="flex flex-col gap-2">
              <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Date / Duration</label>
              <input
                type="text"
                name="date_en"
                required
        ...
        {/* Image */}
        <div>
          <h2 className="text-lg font-bold text-navy border-b border-navy/5 pb-2 mb-4">Media</h2>
          <div className="flex flex-col gap-2">
            <label className="text-navy/70 text-xs font-bold tracking-widest uppercase">Foto Dokumentasi</label>
            <div className="relative border-2 border-dashed border-navy/20 rounded-xl p-4 flex flex-col items-center justify-center gap-2 hover:bg-navy/[0.02] transition-colors min-h-[160px] md:w-1/2">
              {imagePreview ? (
